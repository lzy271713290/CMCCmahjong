...
- 验证状态：`ui-voice-v18` 已通过 103 项自动测试、TypeScript 生产构建和本地联机/后台/四真人整局冒烟；阿里云已启用 Redis 持久化、PM2 守护和后台管理。
...
- 结算/投掷/聊天/全屏优化：每局结算增加胜/负/平风格卡片；投掷改为下拉框选择拖鞋/鸡蛋/土豆并直接砸中目标头像；聊天入口只保留自己头像下方，公屏文字放大；全屏按钮改为图标；摸牌与出牌同帧时先播摸牌。
- 牌桌布局：四面牌墙按真实剩余数显示，自己手牌与落地牌左右分列，出牌提示上移不遮挡手牌。
- 智能测试玩家与分段牌墙：测试玩家会自动吃/碰/杠/胡，并在自回合执行自摸、暗杠、加杠、特殊杠和涨毛；牌墙按四个座位分段消耗，普通摸牌先清空当前段，补牌从尾部段递减。
