...
正常 JSON 日志应包含 `"modelVersion":"governance-ready-v13"`、`"wallRemaining":83` 和 `"totalTiles":136`；`handTileCounts` 中应恰好一家14张、三家13张。日志不会记录具体手牌或暗杠牌面。
...
应返回 `"ok":true`、`"modelVersion":"governance-ready-v13"`、8位 `instanceId` 和运行秒数。
...
```bash
cd mahjong-h5
node scripts/websocket-smoke.mjs ws://服务器公网IP:3000/ws governance-ready-v13
```

命令退出码为0，并返回 `"modelVersion":"governance-ready-v13"`、`"matchRounds":16`、`"gamePhase":"playing"`、`"earlySettlementDuringRoundAllowed":true`、`"earlySettlementPauseDeadline":true`、`"earlySettlementRejectedAndResumed":true`、两轮牌墙递减、公共记录隐私、回放模块和静态美术资源检查，以及 `health.status:200`，表示整场模型、局中解散暂停/拒绝恢复、健康检查、最小回合和资源路由通过。

执行四真人完整一局、投票中断线恢复和最终结算验收：

```bash
node scripts/full-round-smoke.mjs ws://服务器公网IP:3000/ws governance-ready-v13
```
