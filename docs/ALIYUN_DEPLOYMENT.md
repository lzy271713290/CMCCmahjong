...
```bash
grep -E '"event":"(private_hands_distributed|private_hand_restored|tile_discarded|reaction_window_resolved|test_player_auto_discarded|turn_advanced)"' logs/server.jsonl | tail -n 50
```
...
命令退出码为0，并返回 `"gamePhase":"playing"`、`"wallAfterFirstDiscard":82`、`"wallAfterSecondDiscard":81`、`"discardCountAfterSecondTurn":2`，以及两个静态美术资源的正确 MIME/字节数，表示最小回合和横屏牌桌资源的公网流程通过。
