...
正常 JSON 日志应包含 `"modelVersion":"discard-reactions-v2"`、`"wallRemaining":83` 和 `"totalTiles":136`；`handTileCounts` 中应恰好一家14张、三家13张。日志不会记录具体手牌。

验证私有发牌、重连恢复、出牌、自动响应与回合推进监控事件：

```bash
grep -E '"event":"(private_hands_distributed|private_hand_restored|tile_discarded|reaction_options_calculated|reaction_response_received|reaction_window_resolved|meld_claimed|round_ended|test_player_auto_discarded|turn_advanced)"' logs/server.jsonl | tail -n 80
```
...
命令退出码为0，并返回 `"modelVersion":"discard-reactions-v2"`、`"gamePhase":"playing"`、`"wallAfterFirstDiscard":82`、`"wallAfterSecondDiscard":81`、`"discardCountAfterSecondTurn":2`，以及两个静态美术资源的正确 MIME/字节数，表示响应模型、最小回合和横屏牌桌资源的公网流程通过。
