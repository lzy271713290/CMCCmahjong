...
正常 JSON 日志应包含 `"modelVersion":"turn-operations-v3"`、`"wallRemaining":83` 和 `"totalTiles":136`；`handTileCounts` 中应恰好一家14张、三家13张。日志不会记录具体手牌。

验证私有发牌、重连恢复、出牌、自动响应与回合推进监控事件：

```bash
grep -E '"event":"(private_hands_distributed|private_hand_restored|tile_discarded|reaction_options_calculated|reaction_response_received|reaction_window_resolved|meld_claimed|turn_operation_performed|rob_kong_options_calculated|kong_completed|round_ended|test_player_auto_discarded|turn_advanced)"' logs/server.jsonl | tail -n 100
```
...
命令退出码为0，并返回 `"modelVersion":"turn-operations-v3"`、`"gamePhase":"playing"`、`"wallAfterFirstDiscard":82`、`"wallAfterSecondDiscard":81`、`"discardCountAfterSecondTurn":2`，以及两个静态美术资源的正确 MIME/字节数，表示自回合操作模型、最小回合和横屏牌桌资源的公网流程通过。
